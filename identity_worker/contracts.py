...
CONTRACT_VERSION = 'privacy-identity-lora-training-v1'
...
def parse_training_request(event: dict[str, Any]) -> TrainingRequest:
    ...
    if payload.get('execution_mode') != 'controlled_training':
    ...
    return TrainingRequest(payload, _text(payload.get('request_id')), actor_id, run_id, _text(output.get('bucket')), _text(output.get('prefix')))
